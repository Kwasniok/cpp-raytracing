...
void test_runge_kutta_4_delta_constant_velocity() {
    ...
    Vec6 delta_p = runge_kutta_4_delta(f, p0, 1.0);

    Vec3 delta_x = delta_p.first_half();
    Vec3 delta_v = delta_p.second_half();

    TEST_ASSERT_ALMOST_EQUAL_ITERABLE(delta_x, v0, epsilon);
...
void test_runge_kutta_4_delta_constant_force() {
    ...
    TEST_ASSERT_ALMOST_EQUAL_ITERABLE(delta_x, v0 + 0.5 * a, epsilon);
    // v1 = v0 + a * t
    TEST_ASSERT_ALMOST_EQUAL_ITERABLE(delta_v, a, epsilon);
}
