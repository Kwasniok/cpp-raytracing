...
Scene<4_D> make_scene(const SceneConfig& config) {
    ...
    auto camera = std::make_shared<minkowski::PerspectiveCamera>(
        minkowski::make_perspective_camera(position, focus, up_direction,
    ...
    auto emissive_image = make_light_image_material<4_D>(
        config.sky_texture_path, config.sky_texture_gain,
        config.sky_texture_gamma);
...
void render_ppm(const SceneConfig& scene_config, const RenderConfig& config) {
    ...
    Image2D image = renderer->render(geometry, scene);
...
int main(int argc, char** argv) {
    ...
    scene_config.sky_texture_gain =
        parser.get<ColorScalar>("--sky_texture_gain");
    scene_config.sky_texture_gamma =
        parser.get<ColorScalar>("--sky_texture_gamma");
