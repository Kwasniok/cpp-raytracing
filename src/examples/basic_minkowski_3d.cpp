/**
 * @file
 * @brief basic example executable of 3D Euclidean space matching the Minkowski
 *        example but with instant epeed of light
...
std::shared_ptr<cartesian_3d::Sphere3D> make_sphere(const Scalar radius,
                                                    const Vec3 position = {}) {
    auto sphere = std::make_shared<cartesian_3d::Sphere3D>();
 ...
 */
Scene3D make_scene(const Scalar time, const unsigned long number_of_rows) {
    ...
    auto camera =
        std::make_shared<PinholeCamera3D>(cartesian_3d::make_pinhole_camera(
    ...
    // spheres
    const int offset = -1;
    const Scalar total_radius = 1.0;
    if (std::abs(time) < total_radius) {
        const Scalar radius =
            std::sqrt(std::pow(total_radius, 2) - std::pow(time, 2));
        ...
        for (int i{0}; i < static_cast<int>(number_of_rows); ++i) {
            ...
            auto metal_gray = make_metal_volume_checker_material<3_D>(
                0.45 * color, 0.55 * color);
            metal_gray->id.change("metal");

            // left
            {
                auto sphere =
                    make_sphere(radius, {-2.0, 1.0, -2 * (i + offset)});
    ...
    // floor
    const Scalar floor_radius = 1e6;
    if (std::abs(time) < floor_radius) {
        {
            const Scalar radius =
                std::sqrt(std::pow(floor_radius, 2) - std::pow(time, 2));
            auto sphere = make_sphere(radius, {0.0, -radius, 0.0});
            sphere->material = metal_gray;
            scene.add(std::move(sphere));
        }
    }
...
void render_ppm(const RenderConfig& config) {
    ...
    Scene3D scene = make_scene(config.time, config.number_of_rows);
    ...
    renderer->frequent_render_callback =
        [](const typename Renderer3D::State& current_state) {
            cout << "samples: " << current_state.samples << endl;
        };

    renderer->infrequent_render_callback =
        [&config](const typename Renderer3D::State& current_state) {
